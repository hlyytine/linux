...
int module_change_host_page_prot(u64 pfn, enum kvm_pgtable_prot prot, u64 nr_pages)
{
	...
	if (page->host_state & PKVM_MODULE_OWNED_PAGE) {
		...
		ret = ___host_check_page_state_range(
				addr, size, PKVM_PAGE_OWNED, reg, true);
		if (ret)
			goto unlock;
	}
...
int __pkvm_host_reclaim_page_guest(u64 gfn, struct pkvm_hyp_vm *vm, u8 order)
{
	u64 ipa = hyp_pfn_to_phys(gfn);
	size_t page_size;
	kvm_pte_t pte;
	...
	ret = get_valid_guest_pte(vm, ipa, &pte, &phys, order);
	if (ret)
		goto unlock;
...
int __pkvm_host_share_guest(u64 pfn, u64 gfn, struct pkvm_hyp_vcpu *vcpu, u64 nr_pages,
			    enum kvm_pgtable_prot prot)
{
	struct pkvm_hyp_vm *vm = pkvm_hyp_vcpu_to_hyp_vm(vcpu);
	u64 phys = hyp_pfn_to_phys(pfn);
