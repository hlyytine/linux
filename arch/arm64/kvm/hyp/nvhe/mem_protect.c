...
int hyp_check_range_owned(u64 phys_addr, u64 size)
{
	...
	hyp_lock_component();
	ret = __hyp_check_page_state_range((u64)phys_addr,
				  ...
				  size_t size, bool is_memory)
{
	 ...
	 */
	WARN_ON(__host_stage2_set_owner_locked(phys, size, PKVM_ID_GUEST, is_memory, 0, false));
	psci_mem_protect_inc(nr_pages);
...
static int __pkvm_use_dma_locked(phys_addr_t phys_addr, size_t size, struct pkvm_hyp_vcpu *hyp_vcpu)
{
	 ...
	 *   refcounted, taint them by marking them as shared borrowerd, and that
