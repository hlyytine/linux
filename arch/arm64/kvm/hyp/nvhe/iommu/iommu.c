...
#define for_each_drv(drv) \
	for (typeof(kvm_iommu_drivers[0]) *_pp = (kvm_iommu_drivers); \
	     (_pp != &kvm_iommu_drivers[nr_drivers]) && ((drv) = *_pp); _pp++)

/*
 * Drivers are modified from __pkvm_iommu_init, which is only allowed
 * before de-privilege.
 * Instead of paying the lock cost during the runtime, elide the lock
 * after de-privilege.
 */
...
int kvm_iommu_register_ops(struct kvm_iommu_ops *ops, pkvm_handle_t *drv_id)
{
	...
	*drv_id = nr_drivers;
	kvm_iommu_drivers[nr_drivers++] = ops;

	hyp_write_unlock(&kvm_iommu_reg_lock);

	/*
	 * Init may require donation which has to be done outside the reg lock.
	 * That means if a driver failed to init it's just stuck in there.
	 */
	ret = ops->init();
	if (ret)
		return ret;
	...
	WARN_ON(cmpxchg_release(&ops->priv.initialised, false, true));
				 ...
				 enum kvm_pgtable_prot prot)
{
	...
	for_each_drv(kvm_iommu_ops) {
		if (smp_load_acquire(&kvm_iommu_ops->priv.initialised))
...
void kvm_iommu_host_stage2_idmap_complete(bool map)
{
	...
	for_each_drv(kvm_iommu_ops) {
		if (kvm_iommu_ops->host_stage2_idmap_complete &&
		    kvm_iommu_ops->priv.initialised)
