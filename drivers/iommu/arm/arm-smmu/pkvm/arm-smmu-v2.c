		       ...
		       bool is_write, u64 *val)
{
	...
	if (offset >= ARM_SMMU_GR0_SMR(0) &&
		...
		if (is_write) {
			...
			/* Debug: trace SMR writes from host */
			hyp_info("SMR[%u] write: val=0x%x SID=0x%x VALID=%d\n",
				 idx, val32,
				 (u32)FIELD_GET(ARM_SMMU_SMR_ID, val32),
	...
	if (offset >= ARM_SMMU_GR0_S2CR(0) &&
		...
		if (is_write) {
			...
			/* Debug: trace S2CR writes from host */
			hyp_info("S2CR[%u] write: val=0x%08x TYPE=%d CBNDX=%d SMR_VALID=%d\n",
				 idx, val32, requested_type,
				 (int)FIELD_GET(ARM_SMMU_S2CR_CBNDX, val32),
				 smmu->smrs_shadow[idx].valid);
