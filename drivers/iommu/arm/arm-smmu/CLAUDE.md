...
- **ALWAYS consult `docs/design.md`** before making architectural decisions
- This document defines the high-level GPU virtualization strategy and the role of pKVM SMMU

**pKVM SMMU Architecture (Trap-and-Emulate)**:

The architecture has THREE components working together:

1. **arm-smmu-kvm.c (EL1 initialization helper)**:
   - Runs at early boot (`core_initcall`) to set up EL2 SMMU support
   - Passes SMMU instance info (MMIO addresses, sizes) to EL2 via hypercalls
   - Does NOT bind as platform driver - it's not an IOMMU driver replacement
   - Just initializes EL2 state, then gets out of the way
...
2. **arm-smmu.c (standard Linux IOMMU driver)**:
   ...
   - **All its register accesses trap to EL2** (it doesn't know this!)

3. **EL2 arm-smmu-v2.c (hypervisor driver)**:
   - Receives SMMU MMIO via donation (pages unmapped from host stage-2)
   - Traps all host SMMU register accesses via data abort handler
   - Validates and emulates each access
   - Adds Stage-2 translation for DMA isolation
   - Enforces security policy (SID assignments, page table protection)

**Key Point**: arm-smmu-kvm.c and arm-smmu.c BOTH run and serve different purposes:
- arm-smmu-kvm.c: EL2 setup only
- arm-smmu.c: EL1 IOMMU driver (but its MMIO accesses are trapped)
...
#### Issue 3: numpage Hardcoded as 2 (RESOLVED 2025-12-02)

**Symptoms**:
- Kernel panic: "Unable to handle kernel access to hypervisor-protected memory at virtual address ffff800082820000"
- Crash occurs during `arm_smmu_device_reset()` when writing to context bank registers
- CB2 (page 130 at offset 0x820000) incorrectly calculated as "CB128" (out of bounds)

**Root Cause**:
The EL2 driver hardcoded `numpage = 2` in the CB page calculation, but Tegra234 SMMU-500 actually has `numpage = 128`.

ARM SMMUv2 register page layout:
- Pages 0 to numpage-1: GR0/GR1 (global registers)
- Pages numpage to numpage+N-1: Context Banks (CB0 to CB(N-1))

For Tegra234 with 16MB MMIO and 64KB pages:
- numpage = 128 (calculated from ID1.NUMPAGENDXB)
- CB0 = page 128 (offset 0x800000)
- CB127 = page 255 (offset 0xFF0000)

The EL2 driver was computing `cb_idx = page_offset - 2` which gave 128 for page 130, causing an out-of-bounds rejection. The correct calculation is `cb_idx = page_offset - numpage` which gives 2 for page 130.

**Solution**:
1. Added `numpage` field to `struct hyp_arm_smmu_v2_device` in `arm-smmu-v2-shared.h`
2. Calculate numpage from ID1.NUMPAGENDXB: `1 << (FIELD_GET(ARM_SMMU_ID1_NUMPAGENDXB, id1) + 1)`
3. Replace all hardcoded `+ 2` and `- 2` patterns with `+ numpage` and `- numpage`

**Code Locations**:
1. `pkvm/arm-smmu-v2-shared.h:89` - Added `numpage` field
2. `pkvm/arm-smmu-v2.c:366-374` - Added numpage calculation in `smmu_v2_probe_device()`
3. `pkvm/arm-smmu-v2.c:1138-1148` - Fixed entry point check in `smmu_v2_handle_cb()`
4. `pkvm/arm-smmu-v2.c:465-474` - Fixed `smmu_v2_reset()` CB loop
5. All CB handlers: Changed `(cb_idx + 2)` to `page_offset` for secondary base calculations
6. TLB functions: Changed `(cb_idx + 2)` to `(cb_idx + smmu->numpage)`

**Test Result**: 20251202232725 - **SUCCESS**
- No kernel panic
- No EMEM decode errors
- SDMMC (mmc0) initializes successfully
- System boots cleanly

**Code Pattern** (calculation fix):
```c
// OLD (incorrect):
...
6. `20251202224624` - CB crash: numpage hardcoded as 2 (page 130 calculated as CB128)
7. `20251202232725` - **SUCCESS**: numpage fix, SDMMC working, no EMEM errors
...
**Investigation Memos Location**: `${WORKSPACE}/autopilot/results/<timestamp>/investigation_memo.md`

When debugging SMMU issues, investigation memos are created in the autopilot results directory for each test. These contain:
- Problem summary and crash details
- Root cause hypotheses
- Code analysis and flow tracing
- Next steps and findings

**Resolved Investigation (2025-12-02): numpage Hardcoded**

**Final Test**: 20251202232725 - **SUCCESS**

The investigation that started with DABT handler issues was ultimately traced to the `numpage` bug. The chain of issues was:
1. DABT handler returning FALSE (misidentified as main problem)
2. CB register accesses being rejected as "out of bounds"
3. **Root cause**: EL2 hardcoded `numpage = 2` but Tegra234 has `numpage = 128`

See **Issue 3** in "Known Issues (RESOLVED)" section above for complete details.

**Investigation Files** (historical):
- `${WORKSPACE}/autopilot/results/20251202163259/investigation_memo.md` - Original S1PTW=1 analysis
- `${WORKSPACE}/autopilot/results/20251202175411/investigation_memo.md` - DABT handler analysis
- `${WORKSPACE}/autopilot/results/20251202224624/investigation_memo.md` - numpage discovery

**Call Flow (now working correctly)**:
```
handle_host_mem_abort() [mem_protect.c:867-869]
    ↓ (is_dabt && !addr_is_memory)
kvm_iommu_host_dabt_handler() [iommu/iommu.c:214]
    ↓ (kvm_iommu_ops->dabt_handler)
smmu_v2_dabt_handler() [arm-smmu-v2.c:2711]
    ↓
smmu_v2_mmio_handler() → smmu_v2_find_by_mmio_addr()
```

**Debug Logging Added**:
- `iommu/iommu.c:218-226`: Check if kvm_iommu_ops is set
- `arm-smmu-v2.c:2719-2734`: Check if SMMU is found for address
