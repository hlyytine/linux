...
/* TODO: Use nr_pages + pfn with a bitfield? */
...
static int smmu_sync_ste(struct hyp_arm_smmu_v3_device_pv *smmu, u32 sid)
{
	struct arm_smmu_cmdq_ent cmd = {
...
static int smmu_sync_cd(struct hyp_arm_smmu_v3_device_pv *smmu, u32 sid, u32 ssid)
{
	struct arm_smmu_cmdq_ent cmd = {
...
static int smmu_set_identity(pkvm_handle_t iommu, pkvm_handle_t sid, bool on)
{
	...
	if (on) {
		...
		if (dst->data[0] != (STRTAB_STE_0_V |
			FIELD_PREP(STRTAB_STE_0_CFG, STRTAB_STE_0_CFG_S2_TRANS)))
			return -EINVAL;
		if (FIELD_GET(STRTAB_STE_2_S2VMID, dst->data[2]))
			return -EINVAL;
